...
def check_polar_contact_with_ligand_atom(pose, ligand_res, atom_name, max_dist=3.5):
    """
    Returns True if any polar atom from a nearby residue is within `max_dist` Å
    of the given ligand atom (atom_name in ligand_res). Otherwise, False.
    """
    polar_atoms = {"N", "O"}  # Define polar atom types to look for
    ...
    for i in range(1, pose.total_residue() + 1):
        ...
        for j in range(1, res.natoms() + 1):
            atype = res.atom_type(j).element()
            if atype in polar_atoms:
...
def constrain_water_network(pose, water_res, ligand_res):
    ...
    if a86_res and a114_res and c360_res:
        ...
        add_distance_constraint(water_res, "H2", ligand_res, "O3")
        ...
        add_angle_constraint(ligand_res, "O3", water_res, "H2", water_res, "O")
...
def check_constrained_water_hbonds(pose, water_res, ligand_res):
    ...
    # Check H2(water) to O3(ligand)
    hb = get_hbond(water_res, "H2", ligand_res, "O3")
    if hb:
        d = hb.get_HAdist(pose)
        a = hb.get_AHDangle(pose)
        hbond_qualities["water_H2_ligand_O2_quality"] = compute_hbond_quality(d, a)
    else:
        hbond_qualities["water_H2_ligand_O2_quality"] = "N/A"
...
def check_carboxylic_acid_satisfaction(pose, ligand_chain="B", receptor_chain="A",
    ...
    for i in range(1, pose.total_residue() + 1):
        if pdb_info.chain(i) == ligand_chain:
            res = pose.residue(i)
            atoms = [res.atom_name(j).strip() for j in range(1, res.natoms() + 1)]
            if all(ox in atoms for ox in ox_names) and carbon_name in atoms:
                ligand_idx = i
                break
...
def add_hbond_constraint(pose, res1, atom1, res2, atom2, sd=0.2):
    ...
    """
    # get atom indices and current distance
    ...
    # build harmonic function around dist0
...
def main():
    ...
    o1_contact = (
        "yes" if check_polar_contact_with_ligand_atom(pose, lig_res_index, "O1") else "no"
    )
    o2_contact = (
        "yes" if check_polar_contact_with_ligand_atom(pose, lig_res_index, "O2") else "no"
    )
    ...
    # Print water hydrogen bond qualities
