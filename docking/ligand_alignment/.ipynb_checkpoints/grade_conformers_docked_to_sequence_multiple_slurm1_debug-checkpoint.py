...
def aggregate_pass_score_repacked(pass_score_repacked_dirs, num_passes, array_index, final_dir_name='final_pass_score_repacked'):
    """
    Aggregates all pass_score_repacked directories into one final directory.
    The output files are renamed to include the array index and pass number.
    """
    ...
    for pass_idx, source_dir in enumerate(pass_score_repacked_dirs, start=1):
        ...
        for file_name in os.listdir(source_dir):
            if file_name.startswith('repacked') and file_name.endswith('.pdb'):
                ...
                # Create a new file name that includes the array index and pass number
                new_file_name = f"array{array_index}_pass{pass_idx}_{file_name}"
...
def main(argv):
    ...
    # Apply mutations for "docked to sequence"
    ...
    #DEBUG
    # Debug: Show which chain and residue number we are trying to look up.
    print(f"DEBUG: Looking up chain {default['ChainLetter']}, residue {default['ResidueNumber']}")

    # Print the mapping of pose indices to PDB chain and residue numbers.
    for i in range(1, pre_pose.total_residue() + 1):
    pdb_map = pre_pose.pdb_info().pose2pdb(i)
    print(f"Pose index {i}: Chain {pdb_map[0]}, Residue {pdb_map[1]}")
    ...
    # Set up directories for each pass using the SLURM array index to ensure uniqueness
    ...
    for pass_idx in range(1, num_passes + 1):
        ...
        pass_fail_repacked_dir = os.path.join(output_base, f'pass_fail_repacked_{array_index}_{pass_idx}')
        pass_initial_only_repacked_dir = os.path.join(output_base, f'pass_initial_only_repacked_{array_index}_{pass_idx}')
    ...
    aggregate_pass_score_repacked(pass_score_repacked_dirs, num_passes, array_index)
    combined_pkl = os.path.join(output_base, f"combined_array{array_index}_" + (pkl_file_name if pkl_file_name else "combined_results.pkl"))
