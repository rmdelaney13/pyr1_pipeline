...
def add_charge_constraints(pose, ligand_residue_index, weight=1.0):
    """
    Adds electrostatic constraints to the pose based on ligand charges.
    """
    ...
    # Iterate over charged atoms in the ligand
    for ligand_atom_index in range(1, ligand_res.natoms() + 1):
        ligand_atom_name = ligand_res.atom_name(ligand_atom_index).strip()
        ligand_charge = ligand_res.atomic_charge(ligand_atom_index)
        if abs(ligand_charge) > 0.01:  # Only consider atoms with non-zero charges
            ligand_atom_id = AtomID(ligand_atom_index, ligand_residue_index)

            # Add constraints with nearby protein atoms
