...
import time  # Add the time module for tracking execution time
...
def design(pose, resfile, ref_seq, fnr_bonus):
    ...
    print("Setting up score function...")
    sf = pyrosetta.get_fa_scorefxn()
    sf.set_weight(pyrosetta.rosetta.core.scoring.fa_elec, 2.5)  # Emphasize electrostatics
    ...
    print(f"Score function setup completed in {time.time() - start_time:.2f} seconds")

    # SET UP SCORE BONUSES
    start_time = time.time()
    print("Setting up score bonuses...")
    fnr_bonus = 4
    fnr = pyrosetta.rosetta.protocols.protein_interface_design.FavorNativeResidue(pose, fnr_bonus)
    print(f"FNR bonus: {fnr_bonus}")
    print(f"Score bonuses setup completed in {time.time() - start_time:.2f} seconds")

    # SET UP TASK FACTORY
    start_time = time.time()
    print("Setting up task factory...")
    ...
    print(f"Task factory setup completed in {time.time() - start_time:.2f} seconds")
    ...
    print(f"Number of jumps: {jump_num}")
    print(f"Movemap setup completed in {time.time() - start_time:.2f} seconds")

    # PRE-DESIGN SCORE
    start_time = time.time()
    print("Calculating pre-design score...")
    pre_score = sf(pose)
    print(f"Pre-design score: {pre_score}")
    print(f"Pre-design score calculation completed in {time.time() - start_time:.2f} seconds")

    # SET UP FASTRELAX
    start_time = time.time()
    print("Setting up FastRelax...")
    ...
    fr.max_iter(300)
    print(f"FastRelax setup completed in {time.time() - start_time:.2f} seconds")

    # APPLY FASTRELAX
    start_time = time.time()
    print("Applying FastRelax...")
    fr.apply(pose)
    print(f"FastRelax application completed in {time.time() - start_time:.2f} seconds")

    # POST-DESIGN SCORE
    start_time = time.time()
    print("Calculating post-design score...")
    post_score = sf(pose)
    print(f"Post-design score: {post_score}")
    print(f"Post-design score calculation completed in {time.time() - start_time:.2f} seconds")

    # DETERMINE MUTATIONS
    start_time = time.time()
    print("Determining mutations...")
    ...
    print(f"Mutations determination completed in {time.time() - start_time:.2f} seconds")
