...
def align_to_residue_and_check_collision(pose, res, path_to_conformers, df, pkl_file,
    ...
    for pose_info in conformer_prep.yield_ligand_poses(df=df, path_to_conformers=path_to_conformers,
        ...
        if len(keep_list) == 0:
            ...
            # Save rotated pose to pass-specific directory.
            if output_dirs:
            ...
            if not does_collide:
                ...
                if score < max_pass_score:
                    ...
                    # Immediately copy to final_pass_score_repacked directory.
                    if output_dirs and 'final_pass_score_repacked' in output_dirs:
                        final_dir = output_dirs['final_pass_score_repacked']
                        final_output_path = os.path.join(final_dir, f"final_repacked_{job_id}_par{parallel_index}_{total_confs}.pdb")
                        try:
                            shutil.copyfile(output_path, final_output_path)
                            print(f"Immediately copied {output_path} to {final_output_path}")
                        except Exception as e:
                            print(f"Failed to copy to final directory: {e}")
                else:
...
def aggregate_pass_score_repacked(pass_score_repacked_dirs, num_passes, mutation_index, parallel_index):
    ...
    (Not strictly needed when immediately copying to a final directory, but kept here if additional aggregation is desired.)
    """
...
def main(argv):
    ...
    # Create the final aggregation directory and include it in output_dirs.
    final_pass_score_repacked_dir = os.path.join(output_base, "final_pass_score_repacked")
    os.makedirs(final_pass_score_repacked_dir, exist_ok=True)
    print(f"Created final aggregation directory: {final_pass_score_repacked_dir}")
    ...
    for pass_idx in range(1, num_passes + 1):
        ...
        output_dirs = {
            ...
            'final_pass_score_repacked': final_pass_score_repacked_dir
        }
    ...
    # (Optional) Final aggregation of repacked directories (if needed)
