...
def csv_to_df_pkl(csv_file_name, pkl_file_name, auto, path_to_conformers, pose, target_res, lig_res_num):
    ...
    df["Conformer Range"] = df["Conformer Range"].apply(lambda name: (name.split("_")[0], name.split("_")[1]))
...
def align_to_residue_and_check_collision(pose, res, path_to_conformers, df, pkl_file,
    ...
    for pose_info in conformer_prep.yield_ligand_poses(df=df, path_to_conformers=path_to_conformers,
        ...
        if not pose_info:
            print(f"Conformer missing; skipping")
                ...
                for j in range(1, copy_pose.residue(ligand_res_index).natoms() + 1):
                    ...
                    if atom.element() in ["N", "O"]:
                        ...
                        for i in range(len(water_resid)):
                            ...
                            if atom_coords.distance(water_coord) > lower_water_distance and atom_coords.distance(water_coord) < upper_water_distance:
                                keep_list.append(i)
            count += 1
    ...
    tf = time.time()
    print(f"\nTime taken: {(tf-t0)/60:.2f} minutes")
    print(f"Conformers per minute: {total_confs/(tf-t0)*60:.2f}")
...
def main(argv):
    ...
    # Accept a config file and an optional task index (for SLURM)
    parser.add_argument("config_file", help="Your config file", nargs="?", default="config.ini")
    parser.add_argument("task_index", help="Optional SLURM task index", type=int, nargs="?", default=0)
    ...
    # Perform glycine shave on the pocket using positions from the config file
