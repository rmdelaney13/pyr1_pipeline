        ...
        if len(keep_list) == 0: 
            ...
            if not does_collide:
                ...
                if score < 1100:
                    ...
                    conf.pose.pdb_info().name(f"diazi_pass_score")
                    pmm.apply(conf.pose)

                else: 
                    # View alignment for conformers passing collision check
                    conf.pose.pdb_info().name(f"diazi_fail_score")
                    pmm.apply(conf.pose)
            else: 
...
def main(argv):
    ...
    # diazinon
    # V81Y L87M F108Y M158V F159G A160V
    mutant_sidechain_positions = [79, 85, 106, 156, 157, 158] 
    mutant_sidechain_dict = {79:'Y', 85:'M', 106:'Y', 156:'V', 157:'G', 158:'V'}
