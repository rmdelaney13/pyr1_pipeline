        ...
        if len(keep_list) == 0: 
            ...
            if not does_collide:
                ...
                if score < 1100:
                    ...
                    conf.pose.pdb_info().name(f"menit5_pass")
                    pmm.apply(conf.pose)
                    copy_pose.dump_pdb(f"pass_score_repacked/repacked{total_confs}.pdb")

                else: 
                    # View alignment for conformers passing collision check
                    conf.pose.pdb_info().name(f"menit5_fail")
                    pmm.apply(conf.pose)
...
def main(argv):
    ...
    # menitazene 5
    # K59Q H60P L87V N90S S92I E102K Y120A S122L E141K V163F N167A
    mutant_sidechain_positions = [59, 60, 85, 88, 90, 100, 118, 120, 139, 161, 165] 
    mutant_sidechain_dict = {59:'Q', 60:'P', 85:'V', 88:'S', 90:'I', 100:'K', 118:'A', 120:'L', 139:'K', 161:'F', 165:'A'}
