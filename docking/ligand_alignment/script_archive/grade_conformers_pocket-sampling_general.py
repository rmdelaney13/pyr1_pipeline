...
def align_to_residue_and_check_collision(pose, res, path_to_conformers, df, pkl_file,
    ...
    for pose_info in conformer_prep.yield_ligand_poses(df = df, path_to_conformers = path_to_conformers, post_accepted_conformers = False, ligand_residue = lig_res_num):
        ...
        if len(keep_list) == 0: 
            ...
            if not does_collide:
                ...
                if score < -750:
                    # previous filter was -700, everything before May 22nd used that
                    ...
                    pass_score_conformer_list.append((total_confs, score))

                else: 
                    # View alignment for conformers passing collision check
                    conf.pose.pdb_info().name(f"fail_score")
                    pmm.apply(conf.pose)
                    copy_pose.dump_pdb(f"pass_fail-cutoff_repacked/repacked{total_confs}.pdb")
            else: 
    ...
    with open('pass_score_list.csv', mode='w', newline='') as file:
        ...
        for row in pass_score_conformer_list:
            writer.writerow(row)
...
def main(argv):
    ...
    # # pirimiphos
    # # K59R S92M S122Q F159T A160T
    # mutant_sidechain_positions = [59, 90, 120, 157, 158] 
    # mutant_sidechain_dict = {59:'R', 90:'M', 120:'Q', 157:'T', 158:'T'}
    ...
    # Test for design - keep residue at postions 79, 88, 115, and 116, shave the rest of the pocket, keep all non-pocket residues normal 
    # keep: 59
    # pocket_shave_positions = [60, 61, 62] + [x - 2 for x in [81, 83, 87, 89, 90, 92, 108, 109, 110, 117, 120, 122, 141, 159, 160, 163, 164, 167]]
    ...
    try:
        os.mkdir('./pass_all_repacked')
    except:
        print("didn't make directory pass_all_repacked")
