...
def align_to_residue_and_check_collision(pose, res, path_to_conformers, df, pkl_file,
    ...
    # Setup for repacking with minimal design from resfile 
    tf.push_back(operation.InitializeFromCommandline())
    tf.push_back(operation.IncludeCurrent())
    tf.push_back(operation.NoRepackDisulfides())
    tf.push_back(operation.ReadResfile("3QN1_open-pocket-design-screening.resfile"))

    # Not using these - for repacking only 
    ...
    mm.set_bb(False)
    mm.set_chi(True)
    fr = pyrosetta.rosetta.protocols.relax.FastRelax(sf_all)
    ...
    for pose_info in conformer_prep.yield_ligand_poses(df = df, path_to_conformers = path_to_conformers, post_accepted_conformers = False, ligand_residue = lig_res_num):
        ...
        if len(keep_list) == 0: 
            ...
            # Try repacking with limited design from a resfile 
            
            # task_design = tf.create_packer_task(copy_pose)
            # parse_resfile(copy_pose, task_design, "3QN1_open-pocket-design-screening.resfile")
