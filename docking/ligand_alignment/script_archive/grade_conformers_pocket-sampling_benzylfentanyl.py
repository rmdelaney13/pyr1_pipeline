    ...
    for pose_info in conformer_prep.yield_ligand_poses(df = df, path_to_conformers = path_to_conformers, post_accepted_conformers = False, ligand_residue = lig_res_num):
        ...
        if len(keep_list) == 0: 
            ...
            if not does_collide:
                ...
                copy_pose.dump_pdb(f"pass_all_repacked/repacked{total_confs}.pdb")
                ...
                if score < -700:
                    ...
                    conf.pose.pdb_info().name(f"benzyl_pass_score")
                    pmm.apply(conf.pose)

                else: 
                    # View alignment for conformers passing collision check
                    conf.pose.pdb_info().name(f"benzyl_fail_score")
                    pmm.apply(conf.pose)
            else: 
