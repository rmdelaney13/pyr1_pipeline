...
def align_to_residue_and_check_collision(pose, res, path_to_conformers, df, pkl_file,
    ...
    for pose_info in conformer_prep.yield_ligand_poses(df = df, path_to_conformers = path_to_conformers, post_accepted_conformers = False, ligand_residue = lig_res_num):
        ...
        pert_mover.apply(new_pose)
        new_pose.dump_pdb(f"rotated/rotated{total_confs}.pdb")
        print('finish conformer rotation')
        ...
        new_pose.dump_pdb(f"repacked/repacked{total_confs}.pdb")
        print("finished repacking")

        # Extract the new ligand coordinates and update coordinates of conf
        ligand_res_index = location + 1
        for atom_id in range(1, new_pose.residue(ligand_res_index).natoms() + 1):
            atom_coords = new_pose.residue(ligand_res_index).xyz(atom_id)
            conf.pose.residue(1).set_xyz(atom_id, atom_coords)

        # conf.pose.dump_pdb(f"conf{total_confs}.pdb")

        # Check for collision
        new_pose.delete_residue_slow(ligand_res_index)
        new_pose.dump_pdb(f"grid/grid{total_confs}.pdb")
        grid = collision_check.CollisionGrid(new_pose, bin_width = bin_width, vdw_modifier = vdw_modifier, include_sc = include_sc)
