...
def align_to_residue_and_check_collision(pose, res, path_to_conformers, df, pkl_file,
    ...
    for pose_info in conformer_prep.yield_ligand_poses(df = df, path_to_conformers = path_to_conformers, post_accepted_conformers = False, ligand_residue = lig_res_num):
        ...
        keep_list = []
        while len(keep_list) == 0:  # pick first conformers perturbation that satisfies criteria 
            ...
            jump_num = 2
            rotation = 10     # mean rotation angle in degrees 
            ...
            # round 2 r-16 t=0.5
            # round 3 r=20 t=0.5
            # need to try this with 4WVO structure to test 2nd water alignment

            

    
            # can we calculate the angle and distance between the H-bond acceptor and water centroid, then screen on that?

            # Check if new ligand alignment could make H bond
            # should write which atom to use from alignment into the config file instead of hardcoding it here 

            # print(new_pose.residue(ligand_res_index))
            # print(new_pose.residue(new_pose.total_residue()))

            water_resid = []
            # need to get coordinates of TP3 residues, keep them in a list that I could iterate through to accomodate single or multiples
            ...
            # Need to define a line here to describe the direction in which the H-bonding atom is pointing 
            # the line must be through the atom is is attached to next (the carbon)
            ...
            for j in range(1, copy_pose.residue(ligand_res_index).natoms() + 1):
                ...
                if atom.element() == "N" or atom.element() == "O":
                    ...
                    # # Ideally I want to be able to determine if the atom is both the correct distance away and at the right angle to make a hydrogen bond
                    # # however, the angle thing seems to be difficult, so focusing on the distance between atom and water

                    # # Initialize variables to store closest atom information to get vector for angle 
                    # closest_atom_index = None
                    # closest_distance = float('inf')

                    # atom_coords = new_pose.residue(ligand_res_index).xyz(j)
                    # print(atom_coords)
                    # for k in range(1, new_pose.residue(ligand_res_index).natoms() + 1):
                    #     if j == k:
                    #         continue
                    #     other_atom_coords = new_pose.residue(ligand_res_index).xyz(k)
                    #     distance = atom_coords.distance(other_atom_coords)

                    #     if distance < closest_distance:
                    #         closest_distance = distance
                    #         closest_atom_index = k

                    # closest_atom = new_pose.residue(ligand_res_index).xyz(k)
                    # print(closest_atom)

                    # # # determine angle between these there atoms 
                    # # angle_degrees = pyrosetta.numeric.angle_degrees(atom_coords, closest_atom, water_resid[0])
                    # # print(angle_degrees) # this rosetta function doesn't exist, need to update 
                    ...
                    for i in range(len(water_resid)): 
                        ...
                        if distance_to_water < 4.0 and distance_to_water > 2.0:
                            keep_list.append(i)


        copy_pose.dump_pdb(f"rotated{total_confs}.pdb")
