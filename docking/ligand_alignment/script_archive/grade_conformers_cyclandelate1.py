        ...
        if len(keep_list) == 0: 
            ...
            if not does_collide:
                ...
                if score < 1100:
                    ...
                    conf.pose.pdb_info().name(f"cyc1_pass_score")
                    pmm.apply(conf.pose)

                else: 
                    # View alignment for conformers passing collision check
                    conf.pose.pdb_info().name(f"cyc1_fail_score")
                    pmm.apply(conf.pose)
            else: 
...
def main(argv):
    ...
    # cyclandelate sequence 1
    # H60P N90S Y120G A160L N167S
    mutant_sidechain_positions = [60, 88, 118, 158, 165] 
    mutant_sidechain_dict = {60:'P', 88:'S', 118:'G', 158:'L', 165:'S'}
