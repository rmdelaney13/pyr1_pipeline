        ...
        if len(keep_list) == 0: 
            ...
            if not does_collide:
                ...
                if score < 1100:
                    ...
                    conf.pose.pdb_info().name(f"mandi_pass_score")
                    pmm.apply(conf.pose)

                else: 
                    # View alignment for conformers passing collision check
                    conf.pose.pdb_info().name(f"mandi_fail_score")
                    pmm.apply(conf.pose)
            else: 
...
def main(argv):
    ...
    # mandipropamid
    # Y58H K59R V81I F108A S122G F159L
    mutant_sidechain_positions = [58, 59, 79, 106, 120, 157] 
    mutant_sidechain_dict = {58:'H', 59:'R', 79:'I', 106:'A', 120:'G', 157:'L'}
