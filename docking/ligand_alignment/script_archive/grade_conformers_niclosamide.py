        ...
        if len(keep_list) == 0: 
            ...
            if not does_collide:
                ...
                if score < 1100:
                    ...
                    conf.pose.pdb_info().name(f"nic_pass_score")
                    pmm.apply(conf.pose)

                else: 
                    # View alignment for conformers passing collision check
                    conf.pose.pdb_info().name(f"nic_fail_score")
                    pmm.apply(conf.pose)
            else: 
...
def main(argv):
    ...
    # niclosamide
    # K59S H60P N90S E141V
    mutant_sidechain_positions = [59, 60, 88, 139] 
    mutant_sidechain_dict = {59:'S', 60:'P', 88:'S', 139:'V'}
