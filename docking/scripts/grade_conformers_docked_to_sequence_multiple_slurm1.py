...
def align_to_residue_and_check_collision(
    ...
    for pose_info in conformer_prep.yield_ligand_poses(
        ...
        count = 0
        while not keep_candidate and count < 10:
        ...
        if not keep_candidate or copy_pose is None:
            print("Conformer failed backbone clash and H-bond geometry checks")
            total_confs += 1
            continue
        ...
        if output_dirs:
            ...
            rotated_path = os.path.join(rotated_dir, f"rotated{total_confs}.pdb")
        else:
            rotated_path = os.path.join("rotated", f"rotated{total_confs}.pdb")
        ...
        if does_collide:
            if output_dirs:
                ...
                out_path = os.path.join(repacked_dir, f"repacked{total_confs}.pdb")
            else:
                out_path = os.path.join("pass_initial_only_repacked", f"repacked{total_confs}.pdb")
        ...
        if score >= max_pass_score:
            ...
            if output_dirs:
                ...
                out_path = os.path.join("pass_fail_repacked", f"repacked{total_confs}.pdb")
            dpu.dump_pose_pdb(copy_pose, out_path, rename_water=rename_water_to_tp3)
            total_confs += 1
            continue
        ...
        if cluster_idx is None:
            ...
            if output_dirs:
                ...
                out_path = os.path.join(repacked_dir, f"repacked_cluster{cluster_id:04}.pdb")
            else:
    ...
    print(f"Conformers per minute: {total_confs/(tf-t0)*60 if (tf-t0) > 0 else 0:.2f}")
...
def aggregate_pass_score_repacked(
    ...
    """
    Aggregate pass_score_repacked_* directories into a single directory next to pass_ base directory.
    """
    ...
    if os.path.isdir(final_dir):
        final_unique_clustered_docks = len(
            [
                ...
                if name.startswith("array") and name.endswith(".pdb")
            ]
        )
...
def main(argv):
    ...
    for i in range(1, post_pose.total_residue() + 1):
        if i in mutant_sidechain_positions:
            pyrosetta.toolbox.mutate_residue(post_pose, i, mutant_sidechain_dict[i])
    post_mutate_path = "post_mutate.pdb"
    ...
    res = pre_pose.residue(
        pre_pose.pdb_info().pdb2pose(default["ChainLetter"], int(default["ResidueNumber"]))
    )
    ...
    try:
        ...
        print(".pkl file not found, generating one from CSV")
    ...
    for pass_idx in range(1, num_passes + 1):
        ...
        pass_fail_repacked_dir = os.path.join(
            output_base, f"pass_fail_repacked_{array_index}_{pass_idx}"
        )
        pass_initial_only_repacked_dir = os.path.join(
            output_base, f"pass_initial_only_repacked_{array_index}_{pass_idx}"
        )

        for directory in [
            rotated_dir,
            pass_score_repacked_dir,
            pass_fail_repacked_dir,
            pass_initial_only_repacked_dir,
        ]:
    ...
    final_dir = aggregate_pass_score_repacked(
        pass_score_repacked_dirs, num_passes, output_base, array_index
    )
    write_summary_report(output_base, array_index, "sequence", pass_summaries, final_dir)
    combined_pkl = os.path.join(
        output_base,
        f"combined_array{array_index}_" + (pkl_file_name if pkl_file_name else "combined_results.pkl"),
    )
