...
Only pocket-floor anchor residues are used for superposition — the gate loop
is excluded because it has a completely different conformation.
...
Usage:
    ...
    # Batch mode
    python place_ligand.py \
        --threaded-dir outputs/threaded_relaxed/ \
        --boltz-dir inputs/boltz_predictions/ \
        --alignment alignment_map.json \
        --csv ../analysis/boltz_LCA/md_candidates_lca_top100.csv \
        --output-dir outputs/open_gate_structures/

    # With Rosetta minimization
    python place_ligand.py \
        --threaded-pdb ... --boltz-pdb ... --alignment ... --output ... \
        --params params/LCA.params --minimize
...
def parse_pdb_atoms(pdb_path, record_types=("ATOM", "HETATM")):
    ...
    Returns list of dicts with keys: record, serial, name, resname, chain,
    resnum, x, y, z, occupancy, bfactor, element, line (original)
    """
    atoms = []
...
def get_ca_coords(atoms, resnums, chain="A"):
    ...
    Returns dict {resnum: np.array([x, y, z])}
    """
    ca_coords = {}
...
def get_ligand_atoms(atoms, chain="B"):
    ...
    Returns list of atom dicts with coordinates.
    """
    lig_atoms = []
    for atom in atoms:
...
def kabsch_superpose(coords_mobile, coords_target):
    ...
    Finds R, t such that: target ~ R @ mobile + t

    Args:
        coords_mobile: Nx3 numpy array (coordinates to transform)
        coords_target: Nx3 numpy array (target coordinates)
    ...
    # Handle reflection (ensure right-handed coordinate system)
...
def clash_check(protein_atoms, ligand_atoms, severe_threshold=1.5, mild_threshold=2.0):
    ...
    Returns dict with n_severe, n_mild, min_distance, details.
    """
    # Get protein heavy atom coordinates
    prot_coords = []
    ...
    # Using broadcasting: (N_lig, 1, 3) - (1, N_prot, 3) -> (N_lig, N_prot)
...
def write_merged_pdb(protein_atoms, ligand_atoms, conect_records, output_path):
    ...
    Renumbers atoms sequentially. Preserves CONECT records with updated serials.
    """
    lines = []
    serial = 1
    old_to_new_serial = {}
    ...
    for atom in ligand_atoms:
        old_to_new_serial[atom["serial"]] = serial
        # Format atom name with proper alignment
        name = atom["name"]
        if len(name) < 4:
            name = f" {name:<3s}"
        line = (
...
def minimize_complex(output_pdb, params_path, pocket_distance=6.0):
    ...
    Short minimization to resolve minor clashes.
    """
    try:
        import pyrosetta
    ...
    # Find ligand residue (last residue or HETATM)
    lig_res = None
    ...
    if lig_res is None:
        logger.warning("  No ligand residue found in pose — skipping minimization")
        return
    ...
    # Minimize ligand + pocket
    mm = MoveMap()
    mm.set_bb(False)
    mm.set_chi(False)
    for res_idx in pocket_residues:
        mm.set_chi(res_idx, True)
    # Allow ligand rigid-body movement (jump)
    mm.set_jump(True)
...
def process_single(
    ...
    # Get anchor positions (Boltz and 3K3K numbering)
    anchor_boltz = alignment_map["anchor_positions_boltz"]
    anchor_3k3k = alignment_map["anchor_positions_3k3k"]

    # Get CA coordinates for anchor residues
    threaded_ca = get_ca_coords(threaded_atoms, anchor_3k3k, chain=threaded_chain)
    boltz_ca = get_ca_coords(boltz_atoms, anchor_boltz, chain=boltz_chain)

    # Find common anchors (both must have the position)
    common_anchors = []
    for ab, ak in zip(anchor_boltz, anchor_3k3k):
        if ab in boltz_ca and ak in threaded_ca:
            common_anchors.append((ab, ak))

    if len(common_anchors) < 4:
        logger.error(f"  {pair_id}: Only {len(common_anchors)} common anchors — need at least 4")
        return {"pair_id": pair_id, "status": "FAIL_ANCHORS"}

    # Build coordinate arrays for Kabsch
    ...
    # Get CONECT records from Boltz PDB
    ...
    if do_minimize and params_path:
        ...
        logger.info("  Ensure your MD equilibration protocol includes a minimization step")
...
def main():
    parser = argparse.ArgumentParser(
        description="Place ligand from Boltz prediction into open-gate structure"
    )
    ...
    parser.add_argument("--output",
                        help="Output path for the merged open-gate + ligand PDB")
